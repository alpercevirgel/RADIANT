# Dependencies for RADIANT_usr_v1.2
...
# I. Pre-processing ----
  ...
  reqPackages = c("readxl", "flowCore", "flowViz", "ggplot2", "tidyverse", "flowStats", "DelayedArray", "xlsx", "Rcpp", "limma", 
                  ...
                  "digest", "umap", "clustRcheck", "Rphenograph", "colorRamp2")
  
  cranPackages = c("readxl", "ggplot2", "tidyverse", "xlsx", "Rcpp", "RColorBrewer", "matrixStats", "dplyr", "dbplyr", "cowplot", "knitr", 
                   "pdftools", "metR", "gridExtra", "reshape2", "ggpubr", "pheatmap", "tidyr", "BiocManager", "rstatix", "colorRamp2")
  ...
  mapDirectories <- function() {
    ...
    assign("asm", file.path(wd, cellType, sprintf("%s Catalyst %s ArcSinH mapping.xlsx", expID, cellType)), envir = .GlobalEnv)
  ...
  ##write ArcsinH mapping file
  writeAsinHMapping = function(){
    if (write.ArcsinH.Mapping == TRUE){
      if(write.ArcsinH.Mapping & file.exists(asm)){
        userInput = readline(prompt = "Do you want to overwrite the existing ArcsinH Mapping file? (y/n) ")
        
        if(userInput == "n"){
          stop("ArcsinH Mapping file not written. You can continue with the already existing file, or delete it and run this function again to create a new file.")
        }
      }
      ...
      assign("markers", c(colnames(fs)), envir = .GlobalEnv)
      write.xlsx(data.frame(index_values = 1:length(markers), fcs_colname = markers, antigen=tempMarkers, run_asinh=vector(length=length(markers)), cofactor = rep(200, length(markers))), asm, row.names = FALSE)
      message ("ArcsinH mapping file successfully written. You can now indicate to which channels ArcsinH transormation should be applied by writing 'TRUE' or 'FALSE' in the 'run_asinh' column. You can input the cofactor in the 'cofactor' column.\n")
    }
  }
  ...
  variableChecks = function(){
    ...
    stopifnot(transformationMethod %in% c("Logicle", "ArcSinH", "None"))
    ...
    if(cleaningMethod == "PeacoQC" & file.exists(file.path(wd, cellType, "01 FCS files","02 cleaned and transformed files"))){
       ...
       userInput = readline(prompt = "You already started and/or finnished cleaning before, do you want to overwrite these files? (y/n) ")
  ...
  AsinHMapCheck = function(){
    
    asinhMap = read.xlsx(asm,1, stringsAsFactors=FALSE) 
    assign("asinhMap", asinhMap, envir = .GlobalEnv)
    
    
    if (length(asinhMap$fcs_colname) == length(markers)){##checks if length of supplied panel matches fcs file panel
      if (all(asinhMap$fcs_colname %in% markers) == TRUE){##checks if markers in supplied channel are all present in fcs file panel
        mismatch_position = c()
        target_position = c()
        for (i in 1:length(markers)){
          if (asinhMap$fcs_colname[i] == markers [i]){##checks if position of supplied channels match position of fcs file channels
            ...
            target_position = append(target_position, ((which(markers == asinhMap$fcs_colname[i]))/10))##...and the target position is stored
          }
        }
        if (length(mismatch_position) == 0){
          message("asinhMap match successfully validated\n")
        } else {##if there are any position mismatches...
          asinhMap$Target = target_position##...the target positions are appended to the dataframe...
          asinhMap = arrange(asinhMap, Target)##...and the dataframe is adjusted so that the positions match with the fcs file...
          assign(asinhMap, subset(asinhMap, select = -c(Target)), envir = .GlobalEnv)
          message("asinhMap match successfully validated. Panel order has been modified to match fcs file\n")
        }
      } else {##if the supplied channels do not match the fcs file channels...
        mismatch=c(asinhMap$fcs_colname %in% markers)##...a list is made of matches and mismatches
        mismatch_channel = c(which(mismatch == "FALSE"))##positions of mismatches are determined
        cat("supplied asinhMap does not match fcs file panel\n")
  ...
  cleanAndTransform = function(){
    ...
    if (cleaningMethod == "None" & transformationMethod == "None"){
      ...
      if(transformationMethod == "Logicle" | transformationMethod == "ArcSinH"){ dir.create(trns, recursive = TRUE, showWarnings = FALSE) }
      ...
      #import ArcsinH mapping file and perform checks
      if(transformationMethod == "ArcSinH"){
        AsinHMapCheck()
      }
      ...
      for (file in files){
        ...
        }else if(transformationMethod == "ArcSinH"){
          ff_t = ff_c
          asinh.chnl.list = c(asinhMap$fcs_colname)[which(asinhMap$run_asinh == TRUE)]
          for(chnls in asinh.chnl.list){
            asinh.values = asinh(matrix(exprs(ff_t[,chnls])) / asinhMap$cofactor[which(asinhMap$fcs_colname == chnls)])
            asinh.values = matrix(asinh.values)
            exprs(ff_t)[,which(colnames(ff_t) == chnls)] = asinh.values
          }
          cat("\n")
          print("Reference file successfully processed")
        }
...
# II. FlowSOM clustering ----
  ...
  performClustering = function(){
    ...
    message("Performing FlowSOM clustering")
    cat("\n")
    ...
    if(performPG == TRUE){
      message("Performing PhenoGraph clustering")
      cat("\n")
      exprsVector = vector("list", length = length(type_markers(sce)))
      for(i in 1:length(type_markers(sce))){ #for every marker set to type in the panel file...
        exprsVector[[i]] = assay(sce, "exprs")[type_markers(sce)[i],]  #...add the expression data to a list
      }
      exprsData = do.call(cbind, exprsVector) # append all expression data to a matrix
      
      Rphenograph_out <- Rphenograph(exprsData, k = PGnn) # perform phenograph
      
      qs <- c(rowQuantiles, colQuantiles)[[1]]
      qs <- qs(exprsData, probs = c(0.01, 1-0.01))
      qs <- matrix(qs, ncol = 2)
      exprsData <- switch(1,
                          "1" = (exprsData - qs[, 1]) / (qs[, 2] - qs[, 1]),
                          "2" = t((t(exprsData) - qs[, 1]) / (qs[, 2] - qs[, 1])))
      exprsData[exprsData < 0 | is.na(exprsData)] <- 0
      exprsData[exprsData > 1] <- 1
      
      exprsData = cbind(factor(membership(Rphenograph_out[[2]])), exprsData)
      exprsData = aggregate(exprsData[,-1], by = list(pgCluster = exprsData[,1]), FUN = median)
      exprsData = as.matrix(exprsData[, -1])
      colnames(exprsData) = type_markers(sce)
      rownames(exprsData) = 1:nrow(exprsData)
      ...
      ComplexHeatmap::Heatmap(
        ...
        #column_title = "Title_test",
        col = colorRamp2(
          seq(min(exprsData), max(exprsData), l = n <- 100),
          colorRampPalette(rev(brewer.pal(11, "RdYlBu")))(n)),
        border = FALSE,
        row_title = "Cluster_ID",
        row_title_side = "left",
        column_title = "PhenoGraph Heatmap",
  ...
  plot_sequences_DR <- function() {
    ...
    #if("FlowSOM" %in% clusterMethod){
     # dir.create(path = file.path(rsl, "00 Cluster number determination/00 FlowSOM"))
      #for(folder in main_folders){ dir.create(path  = file.path(rsl, "00 Cluster number determination/00 FlowSOM/", folder), recursive = T, showWarnings = F) }
    #}
    #if("PhenoGraph" %in% clusterMethod){
     # dir.create(path = file.path(rsl, "00 Cluster number determination/00 FlowSOM"))
      #for(folder in main_folders){ dir.create(path  = file.path(rsl, "00 Cluster number determination/00 Phenograph/", folder), recursive = T, showWarnings = F) }
    #}
    
    #print flowSOM expression heatmaps
    
    # Create main folders
    dir.create(path = file.path(rsl, "00 Cluster number determination"))
   ...
   # if(performPG == TRUE){
     ...
     # dir.create(path = file.path(rsl, "00 Cluster number determination/00 FlowSOM"))
      ...
      #phenograph_folder = paste0(rsl, "00 Cluster number determination/00 Phenograph/02 Expression heatmap per cluster")
      #nr = max(unique(membership(Rphenograph_out[[2]])))
      #pdf(file = paste0(phenograph_folder, "/Expression_Heatmap_", nr, ".pdf"), width = 12, height = 10, title = "Expression_Heatmap_merged")  
      #print(plotExprHeatmap(sce, k = paste0("meta",nr), bin_anno = TRUE, row_anno = TRUE, row_clust=TRUE, col_clust = FALSE, bars = TRUE, perc = TRUE, by=c("cluster_id"), fun = c("median")))
      #dev.off()      
